...
class FetchAdInsightsByDeviceView(APIView):
    def get(self, request):
        ...
        created_at = 'null'
        if FBAdsInsightByDevice.objects.filter(email=email).exists():
            # Fetch the latest insight for the given email
            latest_insight = FBAdsInsightByDevice.objects.filter(email=email).latest('data_created_date')
        ...
        if created_at != 'null':  # Check if created_at is not null
            ...
            FBAdsInsightByDevice.objects.filter(email=email).delete()
            now = datetime.today()
