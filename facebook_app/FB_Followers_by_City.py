...
class FetchFollowersCity(APIView):
    def get(self, request):
        ...
        created_at = 'null'
        if FacebookFollowersbycity.objects.filter(email=email).exists():
            # Fetch the latest insight for the given email
            latest_insight = FacebookFollowersbycity.objects.filter(email=email).latest('data_created_date')
            created_at = latest_insight.data_created_date
        
        if created_at != 'null':  # Check if created_at is not null
            # delete existing data
            FacebookFollowersbycity.objects.filter(email=email).delete()
            now = datetime.today()
            # Subtract 93 Days (approximately 3 years)
            sincedates = now - timedelta(days= 2)  # Rough estimate (93 Days)
        ...
        for metric_data in analytics_data['data']:
            for value in metric_data['values']:
                ...
                page_fans_city = value['value']

                for city, count in page_fans_city.items():
                    # Save each city and count in the database
                    FacebookFollowersbycity.objects.create(
                        page_id=page_id,
                        email=email,
                        city=city,
